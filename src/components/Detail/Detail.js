...
export const Detail = ({ resource }) => {
    ...
    return <div>
        {Object.keys(mappedResource).length > 0 && <Fragment>
            <Title level={4}>{renderText(mappedResource.title)}</Title>
            <div className="image">
                <img alt={renderText(mappedResource.title)} src={renderImage(mappedResource.poster)} />
            </div>
            <Descriptions title="" bordered>
                {mappedResource.description.map((desc) => {
                    return <Descriptions.Item key={desc.label} label={desc.label}>{renderText(desc.value)}</Descriptions.Item>
                })}
            </Descriptions>
            <div className="extra-details">
                ...
                <div className="results-row">
                    {mappedResource.extraDetails.data.map((item => {
                        return <Card key={item.id} type={mappedResource.extraDetails.type} data={item} />
                    }))}
                </div>
            </div>
        </Fragment>}
